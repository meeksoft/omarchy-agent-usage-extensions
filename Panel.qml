...
Panel {
  ...
  function providerTabName(name) {
    ...
    if (text.length <= 12 || text.indexOf(" ") < 0) return text
    var words = text.split(" ")
    var best = text
    var smallestDifference = text.length
    for (var i = 1; i < words.length; i++) {
      var left = words.slice(0, i).join(" ")
      var right = words.slice(i).join(" ")
      var difference = Math.abs(left.length - right.length)
  ...
  Main {
    ...
    updaterCommand: root.bundledUpdater()
  }
          ...
          Grid {
            ...
            Repeater {
              ...
              Button {
                ...
                text: root.providerTabName(modelData.providerName)
